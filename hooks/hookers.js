      ...
      sys.copyRecursiveSync(staticFolder, targetStaticFolder)
		...
		makeDevServerChanges() {
			let defer = new Q.defer(),
				...
				defaultCsp = `default-src *; script-src 'self' data: 'unsafe-inline' 'unsafe-eval' http://LOCIP:PORT; object-src 'self' data: http://LOCIP:PORT; style-src 'self' 'unsafe-inline' data: ; img-src *; media-src 'self' data: http://LOCIP:PORT; frame-src 'self' data: http://LOCIP:PORT; font-src *; connect-src 'self' data: http://LOCIP:PORT`,
			...
			$('head').prepend(`<meta http-equiv="content-security-policy" content="${defaultCsp.replace(/LOCIP/g, getRouterIpAddr()).replace(/PORT/g, getPort())}">`)
			$('body').prepend(`<script>const localServerIpPort = '${getRouterIpAddr()}:${getPort()}'</script>`).append(`<script src="cordova.js"></script>`)
      ...
      let wpPath = webpackPath + (os.platform() === 'win32' ? '.cmd' : '')
		...
		startWebpackDevServer() {
			let defer = new Q.defer(),
				outText = '',
				isResultFound = false,
				args = [`"${webpackDevServerPath}"`, '--hot', '--inline', '--env.devserver', `--public ${getRouterIpAddr()}:${getPort()}`],
				run = epipeBombPath

			if( os.platform() === 'win32' ) {
				args = ['--hot', '--inline', '--env.devserver', `--public ${getRouterIpAddr()}:${getPort()}`]
